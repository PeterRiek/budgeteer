...
class DataManager {
  ...
  static Future<double> getAccountBalance(
      String filename, String account) async {
    double bal = 0;
    for (var transaction in await loadTransactions(filename)) {
  ...
  static Future<void> saveAccounts(
      String filename, List<String> accounts) async {
    Data data = await getData(filename);
